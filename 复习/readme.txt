...
034,vue实例上的属性?
    vm.$data 获取data数据
    vm.$options 获取挂载项 Vue实例里的
    vm.$nextTick:获取真实dom可以在下一个事件环中
    vm.$el 获取真实DOM 获取到的可能不是更新后的 可以在下一个事件环中获取更新后的DOM元素
    vm.$watch 监控一个数据
    vm.$set 可以给data中动态的添加数据，前提是只能给data添加数据
    vm.$delete
    ...
